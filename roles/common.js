...
if (!Meteor.roles) Meteor.roles = new Mongo.Collection('roles');
...
Roles = {...Roles, ...{
   ...
   */
	createRole: function(roleName) {
		...
		Meteor.roles.upsert({_id: roleName}, {$setOnInsert: {children: []}});
	},
   ...
   */
	getGroupsForUser: function(/*args*/) {
		if (!getGroupsForUserDeprecationWarning) {
			...
			console && console.warn('getGroupsForUser has been deprecated. Use getScopesForUser instead.'); // eslint-disable-line no-console
		}
   ...
   */
	_resolveUser: function(user, force) {
		...
		if (!_.isObject(user)) {
			user = Meteor.users.findOne({_id: user}, {fields: {roles: 1}});
		}
		else if (force || !_.has(user, 'roles')) {
			user = Meteor.users.findOne({_id: user._id}, {fields: {roles: 1}});
		}
